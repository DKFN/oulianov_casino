...
function createcheckpoints(mapname)
   ...
   for i,v in ipairs(races[mapname]) do
         if i+1 == #races[mapname] then
            local obj = tools.CreateObject(52, races[mapname][i+1][1], races[mapname][i+1][2], races[mapname][i+1][3] , 0, races[mapname][i+1][4], 0, 1, 1, 1)
          table.insert(checkpoints,obj)
         else
            if races[mapname][i+1] then
         local obj = tools.CreateObject(336, races[mapname][i+1][1], races[mapname][i+1][2], races[mapname][i+1][3] , 0, 0, 0, 10, 10, 10)
          table.insert(checkpoints,obj)
            end
         end
   end
...
function changerace()
   ...
   for i,v in ipairs(tools.GetAllPlayers()) do
...
AddEvent("OMG:RACING:OnPlayerJoin", function(ply)
   -- if GetPlayerCount()<=16 then
      SetPlayerSpawnLocation(ply, racing_spawns[racesnumbers[currace]][2][1], racing_spawns[racesnumbers[currace]][2][2], 0, racing_spawns[racesnumbers[currace]][1])
      SetPlayerRespawnTime(ply, 500)
      if not checkpoints then
         changerace()
      else
         Delay(2000, function()
            CallRemoteEvent(ply, "OMG:PLAYER_LOAD_DONE")
            SetPlayerHealth(ply, 0)
            --CallRemoteEvent(ply, "OMG:Notify:Message", "A Race is in progress, you will join the game at the end of the race")
            AddPlayerChat(ply, "A Race is in progress, you will join the game at the end of the race")
         end)
      end
   -- else
...
function spawnveh(ply,id,first)
   ...
   local veh = OMG._.CreateVehicle(RACING_GMID, id, px, py, pz , h)
...
AddEvent("OMG:RACING:OnPlayerSpawn", function(ply)
   local found = false
...
AddEvent("OMG:RACING:OnPlayerLeaveVehicle",function(ply,veh,seat)
...
AddEvent("OnPackageStart",function()
   ...
   RACING_GMID = OMG.GameManager.Register("RACING", {
      fullName = "Onset Racing",
      author = "Voltaism",
      lobby = {
         maxPlayers = 16,
         npc = {
   ...
   tools = OMG._.GetToolBox(RACING_GMID)
end)
...
AddCommand("showspawns",function(ply)
   if dev then
    for i,v in ipairs(racing_spawns[racesnumbers[currace]]) do
        if i > 1 then
         tools.CreateObject(1363, v[1], v[2], v[3] , 0, racing_spawns[racesnumbers[currace]][1], 0, 1, 1, 1)
        end
    end
   end
end)
